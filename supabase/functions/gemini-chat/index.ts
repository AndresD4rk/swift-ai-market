...
serve(async (req)=>{
  ...
  try {
    ...
    if (searchResponse.data && searchResponse.data.products) {
      ...
      highSimilarityProducts = contextProducts.filter((product)=>product.similarity >= 0.673);
    }
    ...
    if (contextProducts.length > 0) {
      ...
      contextProducts.forEach((product, index)=>{
...
Siempre responde en lenguaje natural y conversacional, pero sin usar símbolos, emojis, listas, ni ningún tipo de formato especial. Usa solo texto plano con oraciones completas.

${productContext}

Si no encuentras productos relevantes para la consulta del usuario, simplemente responde: 
"Parece que no tengo un producto de la tienda que coincida con lo que buscas. ¿Quieres intentar con otra descripción?."
Sé breve, directo y útil.

Si el usuario hace preguntas fuera del contexto de productos o compras, simplemente responde: 
"Lo siento, solo puedo ayudarte con productos de la tienda."
Sé breve, directo y útil.
`;
    ...
    return new Response(JSON.stringify({
      response: aiResponse,
      contextProducts: highSimilarityProducts,
